...
import { getGeminiApiKeys, callGemini, callGeminiWithFallback, GEMINI_MODEL } from '../lib/gemini.js';
...
// The Gemini caller and its constants now live in lib/gemini.js, shared with
// api/staff-chat.js. Re-exported from this file's test-only export block below
...
- On a customer's FIRST message about wall beds, also ask an open, low-pressure question about what they're looking for — e.g. "what's prompting the change?" or "what would make the biggest difference for your space?" — alongside the ceiling/room question, not instead of it. Purpose: some customers already know exactly which model they want, but others haven't figured out what they actually need yet, and this gives them room to say so. Keep it to one short, natural sentence — don't turn it into a form, and don't repeat it once the customer states a clear need or preference (e.g. "for my kid's study room", "need something for guests"), since at that point they've already answered it
...
"WHY IS THIS SO EXPENSIVE" / VALUE QUESTIONS — read the customer's intent first, then pick ONE path:
- Some customers just want a fast, factual answer (terse phrasing, already comparing options, "just wondering", "quick q"). For these: answer directly and briefly — name 2-3 concrete things the price reflects (e.g. the mechanism, materials, mattress included, warranty length) from the knowledge base. Don't pad it with questions or a sales pitch.
- Some customers are really asking "is this worth it for me" / "should I get this" (phrasing like "is it worth it", "not sure if I need this", "what's the difference vs the cheaper one", or general hesitation about the price). For these: still answer what drives the price, but also briefly connect it to whatever you already know about their situation from this conversation (room, ceiling height, what they said they need it for). If you don't know their situation yet, a short, genuine question is fine ("what's most important to you here — saving space, or having the sofa built in?") — but only ask if it would actually change the answer, and only once.
- If the intent is genuinely unclear, default to the direct answer first, then add one short, optional line inviting more ("happy to go into more detail if useful") rather than guessing which path they want.
- Either way: never use price-justification as an excuse to avoid mentioning a cheaper alternative if one exists and fits what they've told you — see the cheaper-alternative rule below.
